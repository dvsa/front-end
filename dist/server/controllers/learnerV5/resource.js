...
function resourceGet(req, res) {
  ...
  if ((0, _validationFunctions.isNumeric)(parseInt(resourceID)) && parseInt(resourceID) >= 1) {
    returnPath = 'prototypes/learner/v5/resource/' + resourceID;
  } else if (availableTypes.includes(resourceType)) {
    returnPath = 'prototypes/learner/v5/resource/' + resourceType;
  } else {
    returnPath = 'prototypes/learner/v5/resource/index';
  }
...
function resourcePost(req, res) {
  ...
  if (resourceID === '15') {
    return res.redirect('/prototypes/learner/v5/scorms/basic-fire-safety');
  } else {
