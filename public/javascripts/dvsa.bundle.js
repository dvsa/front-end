...
var storages = __webpack_require__(58)
...
exports.Accordion = exports.ACCORDION_CONSTANTS = undefined;
...
var _md = __webpack_require__(70);

var _md2 = _interopRequireDefault(_md);

var _smoothScroll = __webpack_require__(72);

var _smoothScroll2 = _interopRequireDefault(_smoothScroll);

var _findIndex = __webpack_require__(73);
...
var ACCORDION_CONSTANTS = exports.ACCORDION_CONSTANTS = {
...
var Accordion = exports.Accordion = function () {
  function Accordion(accordionElement) {
    ...
    this.headerClickHandler = function (event) {
      if (!event.target || !_this.state.sections.length) return;
      var section = (0, _misc.closestParentOfEl)(event.target, '.' + ACCORDION_CONSTANTS.classNames.section);
      if (section.getAttribute(ACCORDION_CONSTANTS.attributeNames.preventDefault)) return;
      var sectionHeaderCategory = section.getAttribute(ACCORDION_CONSTANTS.attributeNames.sectionCategory);
      var stateSectionIndexId = Number(section.getAttribute(ACCORDION_CONSTANTS.attributeNames.stateIndexId));
      var sectionFromState = _this.state.sections[stateSectionIndexId];
      if (!sectionFromState) return;
      var newSectionOpenState = !sectionFromState.sectionOpen;
      _this.state.sections[stateSectionIndexId].sectionOpen = newSectionOpenState;
      _this.refreshState();
      _this.smoothScroll.animateScroll(section, true, _this.smoothScrollOptions);
      _this.pushDataLayerForAccordion(stateSectionIndexId);
    };

    this.expandButtonClickHandler = function (event) {
      _this.state.expanding = true;
      _this.state.expandAll = !_this.state.expandAll;
      if (_this.state.expandAll) {
        $.fire(document, ACCORDION_CONSTANTS.eventNames.expandAllOpen);
      }
      _this.refreshState();
      _this.state.expanding = false;
      _this.smoothScroll.animateScroll(event.target, true, _this.smoothScrollOptions);
      _this.pushDataLayerForAllAccordions();
    };

    this.pushDataLayerForAllAccordions = function () {
      if (!window.dataLayer) return;
      var expandState = _this.state.expandAll ? ACCORDION_CONSTANTS.dataLayer.open : ACCORDION_CONSTANTS.dataLayer.close;
      var dataLayerObject = {
        event: ACCORDION_CONSTANTS.dataLayer.linkClickEvent,
        link: ACCORDION_CONSTANTS.dataLayer.sectionAll,
        'link-text': _this.getExpandButtonText(),
      ...
      _this.state.sections.forEach(function (section) {
        var sectionDataLayerInfo = _this.getSectionDataLayerInfo(section);
    ...
    this.pushDataLayerForAccordion = function (sectionIndex) {
      if (!window.dataLayer || !sectionIndex) return;
      var section = _this.state.sections[sectionIndex];
    ...
    this.pushDataLayerForSavedState = function (sections) {
    ...
    this.getSectionDataLayerInfo = function (section) {
      if (!section) return;
      return {
    ...
    this.uniqueIdentifier = 'js-accordion-' + (0, _md2.default)(this.accordionElement.innerHTML);
    ...
    (0, _misc.toggleClass)(this.accordionElement, ACCORDION_CONSTANTS.classNames.jsEnabled, true);
  ...
  _createClass(Accordion, [{
    ...
    value: function setup() {
      ...
      this.sections.forEach(function (sectionElement) {
        var sectionHeaderElement = sectionElement.querySelector('.' + ACCORDION_CONSTANTS.classNames.header);
        var sectionContentElement = sectionElement.querySelector('.' + ACCORDION_CONSTANTS.classNames.content);
        var sectionUniqueIdentifier = (0, _md2.default)(sectionElement.innerHTML);
        var sectionContentId = sectionHeaderElement.getAttribute(ACCORDION_CONSTANTS.attributeNames.sectionContentId);
        // Add the section elements to the state
        _this2.state.sections.push({
          sectionUniqueIdentifier: sectionUniqueIdentifier,
          sectionElement: sectionElement,
          sectionHeaderElement: sectionHeaderElement,
          sectionContentElement: sectionContentElement,
          sectionContentId: sectionContentId,
          sectionOpen: _this2.isSectionOpen(sectionElement)
        });
        // Update the DOM elements state index ID
        var stateSectionIndexId = _this2.state.sections.length - 1;
     ...
     * @author Tameem Safi <t.safi@kainos.com>
     */
    value: function restoreSavedStateData() {
      var _this3 = this;

      // Restore state if saved
      var savedState = _store2.default.get(this.uniqueIdentifier);
      // Check there is a saved state with sections
      if (!savedState || !savedState.sections) return;
      var restoredSections = [];
      savedState.sections.forEach(function (section) {
        ...
        var sectionIndex = (0, _findIndex2.default)(_this3.state.sections, {
        ...
        var stateSection = _this3.state.sections[sectionIndex];
        ...
        _this3.state.sections[sectionIndex].sectionOpen = savedState.expandAll ? true : section.open;
        // Add current section to restored list
        restoredSections.push(_this3.state.sections[sectionIndex]);
      });
    ...
    key: 'saveCurrentStateData',
    value: function saveCurrentStateData() {
      ...
      _store2.default.set(this.uniqueIdentifier, data);
    }
    ...
    value: function refreshState() {
      ...
      if (this.state.sections) {
        ...
        this.state.sections.forEach(function (section) {
          ...
          if (_this4.state.expanding) {
            // if (!section.sectionElement.getAttribute(ACCORDION_CONSTANTS.attributeNames.disableStateRestore)) {
            section.sectionOpen = _this4.state.expandAll;
            // }
          }

          // Toggle the correct class based on the state
          var sectionOpenState = section.sectionOpen;

          // Add/Remove the open class based on whether the section is open or closed
          (0, _misc.toggleClass)(section.sectionElement, ACCORDION_CONSTANTS.classNames.sectionOpen, section.sectionOpen);
          ...
          // was not clicked
          if (openCount >= 1 && !_this4.state.expanding) {
            _this4.state.expandAll = false;
          }

          // Check if all sections are open and change the expand all state,
          // but only if expand button was not clicked
          if (openCount >= _this4.state.sections.length && !_this4.state.expanding) {
            _this4.state.expandAll = true;
          }
        });
      }
     ...
     */
    value: function isSectionOpen(element) {
      // Check if element has the open class
      return (0, _misc.elHasClass)(element, ACCORDION_CONSTANTS.classNames.sectionOpen) ? true : false;
    }
    ...
    key: 'getExpandButtonText',
    value: function getExpandButtonText() {
...
var _expire = __webpack_require__(67);

var _expire2 = _interopRequireDefault(_expire);

var _shared = __webpack_require__(19);

var _modules = __webpack_require__(68);

var _gds = __webpack_require__(179);
...
(0, _shared.domReady)(function () {
  // GDS
  (0, _gds.initGDS)();

  // Modules
  (0, _modules.initModules)();
});
...
var _accordions = __webpack_require__(69);

var _document = __webpack_require__(178);

var initModules = exports.initModules = function initModules() {
  (0, _accordions.initAccordions)();
  (0, _document.initJavascriptEnabled)();
};
...
exports.initAccordions = undefined;

var _accordion = __webpack_require__(31);

var _recallsAccordion = __webpack_require__(159);
...
var initAccordions = exports.initAccordions = function initAccordions() {
  ...
  if (accordions.length) {
    ...
    accordions.forEach(function (accordion) {
      new _accordion.Accordion(accordion);
    });
  }

  // Initialize ajax for recalls accordion
  new _recallsAccordion.RecallsAccordion();
};
...
(function(){
  var crypt = __webpack_require__(71),
...
exports.RecallsAccordion = exports.RECALLS_ACCORDION_CONSTANTS = undefined;
...
var RECALLS_ACCORDION_CONSTANTS = exports.RECALLS_ACCORDION_CONSTANTS = {
  selectors: {
...
var RecallsAccordion = exports.RecallsAccordion = function () {
  function RecallsAccordion() {
    var _this = this;

    _classCallCheck(this, RecallsAccordion);

    this.recallsHeadingClickHandler = function (event) {
      if (!_this.state.ajaxEndpoint) return;
      // Enable loading
      if (!_this.state.loading && !_this.state.ajaxContentAddedToDOM) {
        // Change the state
        _this.state.loading = true;
        // Toggle loading class to show spinner and message
        (0, _misc.toggleClass)(_this.recallsContentElement, RECALLS_ACCORDION_CONSTANTS.classNames.contentLoading, true);
        // Make AJAX call
        _axios2.default.post(_this.state.ajaxEndpoint, _extends({}, _this.state.ajaxRequestBody)).then(function (response) {
          _this.state.loading = false;
          _this.state.ajaxContentAddedToDOM = true;
          // Remove loading class
          (0, _misc.toggleClass)(_this.recallsContentElement, RECALLS_ACCORDION_CONSTANTS.classNames.contentLoading, false);
          // Add html form ajax to output
          _this.recallsContentOutputElement.innerHTML = response.data;
          // Display the accordion output
          (0, _misc.toggleClass)(_this.recallsContentElement, RECALLS_ACCORDION_CONSTANTS.classNames.contentShowOutput, true);
    ...
    this.parentAccordionElement = (0, _misc.closestParentOfEl)(this.recallsAccordionSectionElement, '.' + _accordion.ACCORDION_CONSTANTS.classNames.accordion);
    this.recallsAccordionContentElement = this.recallsAccordionSectionElement.querySelector('.' + _accordion.ACCORDION_CONSTANTS.classNames.content);
  ...
  _createClass(RecallsAccordion, [{
    key: 'setup',
    value: function setup() {
      // Remove no js class to hide alternative method
      (0, _misc.toggleClass)(this.recallsContentElement, RECALLS_ACCORDION_CONSTANTS.classNames.contentNoJs, false);
      ...
      $.events(document, _defineProperty({}, _accordion.ACCORDION_CONSTANTS.eventNames.expandAllOpen, this.recallsHeadingClickHandler));
    }
...
var initJavascriptEnabled = exports.initJavascriptEnabled = function initJavascriptEnabled() {
...
exports.initGDS = undefined;

var _modules = __webpack_require__(180);

var initGDS = exports.initGDS = function initGDS() {
  (0, _modules.initModules)();
};
...
var _cookieMessage = __webpack_require__(181);

var _headerMenu = __webpack_require__(182);

var _linksWithRole = __webpack_require__(183);

var _showHideContent = __webpack_require__(184);

var initModules = exports.initModules = function initModules() {
  new _cookieMessage.CookieMessage();
  new _headerMenu.HeaderMenu();
  new _linksWithRole.LinksWithRole();
  new _showHideContent.ShowHideContent();
};
...
exports.CookieMessage = undefined;
...
var CookieMessage = exports.CookieMessage = function () {
  function CookieMessage() {
    ...
    this.storeValueForCookieMessage = _store2.default.get(this.cookieMessageStoreKey);
  ...
  _createClass(CookieMessage, [{
    ...
    value: function setup() {
      if (!this.storeValueForCookieMessage) {
        ...
        _store2.default.set(this.cookieMessageStoreKey, 'yes', this.expireDate);
      }
    }
  }]);
...
var HeaderMenu = exports.HeaderMenu = function () {
  function HeaderMenu() {
  ...
  _createClass(HeaderMenu, [{
    ...
    value: function setup() {
      ...
      $.delegate(window, 'click', '.' + this.headerToggleClass, function (e) {
        e.preventDefault();
        var itemsContainerId = $(event.target).getAttribute('href');
        var itemsContainer = $(itemsContainerId);
        (0, _shared.toggleClass)(event.target, _this.headerButtonHiddenClass);
        (0, _shared.toggleClass)(itemsContainer, _this.contentVisibleClass);
      });
    }
  }]);
...
var LinksWithRole = exports.LinksWithRole = function () {
  function LinksWithRole() {
    _classCallCheck(this, LinksWithRole);

    this.roleButtonSelector = '[role="button"]';
    this.setup();
  }

  _createClass(LinksWithRole, [{
    key: 'setup',
    value: function setup() {
      $.delegate(document, 'keydown', this.roleButtonSelector, function (event) {
...
exports.ShowHideContent = undefined;
...
var ShowHideContent = exports.ShowHideContent = function () {
  function ShowHideContent() {
    var _this = this;

    _classCallCheck(this, ShowHideContent);
    ...
    this.clickEventHandler = function (event) {
      ...
      switch (type) {
        case 'radio':
          {
            var radioGroupName = event.target.getAttribute('name');
            // Refresh all radios for this group
            if (radioGroupName) {
              var radioInputsForGroup = $$('input[type="radio"][name="' + radioGroupName + '"');
              radioInputsForGroup.forEach(function (radio) {
                _this.toggleContentBasedOnCheckState(radio);
              });
            }
            return;
          }
        case 'checkbox':
          {
            ...
            _this.toggleContentBasedOnCheckState(event.target);
            return;
          }
        default:
          {
    ...
    this.toggleContentBasedOnCheckState = function (element) {
      if (!element) return;
      var targetInfo = _this.getTargetFromElement(element);
      (0, _shared.toggleClass)(targetInfo.element, _this.contentHiddenClass, !element.checked);

      // Refresh aria tags
      element.setAttribute(_this.ariaControlsAttributeName, targetInfo.id);
      element.setAttribute(_this.ariaExpandedAtributeName, element.checked ? 'true' : 'false');
      targetInfo.element.setAttribute(_this.ariaHiddenAttributeName, element.checked ? 'false' : 'true');
    };
  ...
  _createClass(ShowHideContent, [{
     ...
     */
    value: function resetAllAriaAttributes() {
     ...
     */
    value: function getTargetFromElement(element) {
      if (!element) return;
      var parentContainer = (0, _shared.closestParentOfEl)(element, this.dataTargetSelector);
      var targetContainerId = parentContainer.getAttribute('data-target');
