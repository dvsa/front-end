...
export class DvsaBrakeTestConfigurationClass3Plus {
  ...
  init = () => {
    ...
    if (this.elements.numberOfAxles) {
  ...
  shouldEnableNotApplicableRadioButton = () => {
    const brakeTypeCheck =
      this.state.serviceBrakeTestType === this.constants.ROLLR && this.state.parkingBrakeTestType === this.constants.ROLLR;

    const vehicleClassCheck =
      this.state.data.vehicleClass === '3' || this.state.data.vehicleClass === '4' || this.state.data.vehicleClass === '4A';
