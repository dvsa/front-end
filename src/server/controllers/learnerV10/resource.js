...
export function resourceGet(req, res) {
  ...
  if (isNumeric(parseInt(resourceID))) {
    returnPath = 'prototypes/learner/v10/resource/' + resourceID;
  } else if (availableTypes.includes(resourceType)) {
    returnPath = 'prototypes/learner/v10/resource/' + resourceType;
  } else {
...
export function resourcePost(req, res) {
  ...
  if (resourceID === '15') {
    return res.redirect('/prototypes/learner/v10/scorms/basic-fire-safety');
  } else {
