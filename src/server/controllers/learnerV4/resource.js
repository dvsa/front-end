...
export function resourceGet(req, res) {
  ...
  if (isNumeric(parseInt(resourceID)) && parseInt(resourceID) >= 1) {
    returnPath = 'prototypes/learner/v4/resource/' + resourceID;
  } else if (availableTypes.includes(resourceType)) {
    returnPath = 'prototypes/learner/v4/resource/' + resourceType;
  } else {
    returnPath = 'prototypes/learner/v4/resource/index';
  }
...
export function resourcePost(req, res) {
  ...
  if (resourceID === '15') {
    return res.redirect('/prototypes/learner/v4/scorms/basic-fire-safety');
  } else {
