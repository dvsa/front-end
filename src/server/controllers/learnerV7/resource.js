...
export function resourceGet(req, res) {
  ...
  if (isNumeric(parseInt(resourceID))) {
    returnPath = 'prototypes/learner/v7/resource/' + resourceID;
  } else if (availableTypes.includes(resourceType)) {
    returnPath = 'prototypes/learner/v7/resource/' + resourceType;
  } else {
    returnPath = 'prototypes/learner/v7/resource/index';
  }
...
export function resourcePost(req, res) {
  ...
  if (resourceID === '15') {
    return res.redirect('/prototypes/learner/v7/scorms/basic-fire-safety');
  } else {
