...
export function resourceGet(req, res) {
  ...
  if (isNumeric(parseInt(resourceID))) {
    returnPath = 'prototypes/learner/v9/resource/' + resourceID;
  } else if (availableTypes.includes(resourceType)) {
    returnPath = 'prototypes/learner/v9/resource/' + resourceType;
  } else {
    returnPath = 'prototypes/learner/v9/resource/index';
  }
...
export function resourcePost(req, res) {
  ...
  if (resourceID === '15') {
    return res.redirect('/prototypes/learner/v9/scorms/basic-fire-safety');
  } else {
